...
(defun simulate-game (ev1 ev2 explore-rate1 explore-rate2)
  (let ((board 0)
        moves
        (move-cache (make-array 9 :element-type 'move :initial-element 0)))
    (dotimes (turn 9)
      ...
      (let* ((player (if (evenp turn) +player1+ +player2+))
        ...
        (if (< (random 1d0) explore-rate)
            (let ((len 0))
              (dotimes (i 3)
                (dotimes (j 3)
                  (when (zerop (bref board i j))
                    (setf (aref move-cache len) (pack-move i j))
                    (incf len))))
              (with-unpacking-move (i j) (aref move-cache (random len))
                  ...
                  (when (zerop (bref board i j))
                    ...
                    (when (if (= player +player1+)
                      ...
                      (setq opt-value (aref ev board)
                            ...
                            opt-j j))
                    (setf (bref board i j) 0))))
              (push (pack-move opt-i opt-j) moves)
              (setf (bref board opt-i opt-j) player)))))
    (values board moves)))
...
(defun update (evaluation moves board player step)
  (let* ((value2 (float (detect-winner board) 1d0))
         (board2
           (let ((board board))
             (loop (with-unpacking-move (i j) (car moves)
      ...
      (let ((optimal-value2 (if (= player +player1+) -1d0 1d0)))
        ...
        (when (= optimal-value2 value2)
          (let* ((old (aref evaluation board0))
                 (new (+ old (* step (- value2 old)))))
            ;; (dbg (board-to-line board0) old new)
...
(defun simulate-all (init-step explore-rate n)
